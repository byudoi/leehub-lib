-- ╔══════════════════════════════════════════╗
-- ║           VenomLib  v1.0                 ║
-- ║     UI Library basada en VENOM GUI       ║
-- ║           By LEEH4CK                     ║
-- ╚══════════════════════════════════════════╝
--
-- USO BÁSICO:
--   local Lib = loadstring(game:HttpGet("URL_DE_TU_RAW"))()
--   local Win = Lib:Window({ Title="MiHub", Creator="YourName", Key=Enum.KeyCode.RightShift })
--   local Tab = Win:Tab({ Name="Combat", Icon="rbxassetid://118115903634266" })
--   local Sec = Tab:Section("Silent Aim")
--   Sec:Toggle({ Name="Activar", Default=false, Callback=function(v) end })
--   Sec:Slider({ Name="FOV", Min=30, Max=800, Default=150, Callback=function(v) end })
--   Sec:Button({ Name="Ejecutar", Callback=function() end })
--   Tab:Label("Separador")

local VenomLib = {}
VenomLib.__index = VenomLib
...
--  WINDOW
-- ══════════════════════════════════════════
function VenomLib:Window(cfg)
    cfg = cfg or {}
    local title   = cfg.Title   or "VenomLib"
    local creator = cfg.Creator or ""
    local game_   = cfg.Game    or ""
    local key     = cfg.Key     or Enum.KeyCode.RightShift
    local icon    = cfg.Icon    or ""   -- rbxassetid para el float icon

    -- ScreenGui
    local GUI = New("ScreenGui", {
        Name         = "VenomLib_" .. title,
        ResetOnSpawn = false,
    ...
    -- ── Objeto Window público ──────────────────
    local pages  = {}
    local Window = {}

    -- Tab:Section, Tab:Toggle, Tab:Slider, Tab:Button, Tab:Label, Tab:LineSlider
    function Window:Tab(tabCfg)
        ...
        function Tab:Section(ttl)
            ...
            -- LINE SLIDER (con track arrastrable)
        ...
        -- LABEL directo en el tab (sin section)
